          ...
          if (SOFT_DELETE_MODELS.includes(model)) {
            ...
            if (operation === 'update' || operation === 'updateMany') {
              ...
              if (operation === 'upsert') {
                  ...
                  // Check if a soft-deleted record exists (no deleted_at filter)
                  try {
                    ...
                    if (existing && existing.deleted_at !== null) {
                      // Revive the soft-deleted record with the update data
                      return modelDelegate.update({
                    ...
                    // If the lookup fails, fall through to normal upsert
